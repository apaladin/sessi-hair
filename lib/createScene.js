...
import { COLLECTIONS, STOPS, ROOM, WALL_ART } from './data';
...
export function createScene(canvas, callbacks = {}) {
  ...
  display(COLLECTIONS[0], 0, -ROOM.hd + 0.12, 0, 3);
  display(COLLECTIONS[1], -7.4, -ROOM.hd + 0.12, 0, 4);
  display(COLLECTIONS[2], 7.4, -ROOM.hd + 0.12, 0, 5);
  display(COLLECTIONS[3], -ROOM.hw + 0.12, -1.2, Math.PI / 2, 6);
  display(COLLECTIONS[4], ROOM.hw - 0.12, -1.2, -Math.PI / 2, 7);

  /* --------------------- centerpiece: The Crown --------------------- */
  ...
  const plinth = new THREE.Mesh(
    new THREE.CylinderGeometry(1.15, 1.3, 0.5, 48),
    new THREE.MeshStandardMaterial({ color: 0xfaf5ea, roughness: 0.45 })
  );
  ...
  const ring = new THREE.Mesh(
    ...
    new THREE.MeshStandardMaterial({ color: GOLD, roughness: 0.2, metalness: 0.85 })
  );
  ring.rotation.x = Math.PI / 2;
  ring.position.y = 3.55;
  center.add(ring);

  const strandTones = [0x2a1b10, 0x432c1a, 0x5f4023, 0x8f6535, 0xb98a4c, 0xd9bc8c];
  const strandGroup = new THREE.Group();
  const rndS = mulberry(7);
  for (let i = 0; i < 70; i++) {
    const a = (i / 70) * Math.PI * 2 + rndS() * 0.1;
    const r0 = 0.52 + rndS() * 0.05;
    const r1 = 0.55 + rndS() * 0.55;
    const sway = (rndS() - 0.5) * 0.95;
    const pts = [
      new THREE.Vector3(Math.cos(a) * r0, 3.55, Math.sin(a) * r0),
      new THREE.Vector3(Math.cos(a + sway * 0.4) * (r0 + 0.1), 2.7, Math.sin(a + sway * 0.4) * (r0 + 0.1)),
      new THREE.Vector3(Math.cos(a + sway) * r1, 1.7, Math.sin(a + sway) * r1),
      new THREE.Vector3(Math.cos(a + sway * 1.3) * (r1 * 1.05), 0.62, Math.sin(a + sway * 1.3) * (r1 * 1.05)),
    ];
    const tube = new THREE.Mesh(
      new THREE.TubeGeometry(new THREE.CatmullRomCurve3(pts), 22, 0.007 + rndS() * 0.009, 5),
      new THREE.MeshStandardMaterial({
        color: strandTones[Math.floor(rndS() * strandTones.length)],
  ...
  center.add(strandGroup);
  scene.add(center);
  contactShadow(0, 2.2, 3.4);
  ...
  // gold pendant rings suspended above The Crown
  [[0.58, 4.5], [0.42, 4.85], [0.28, 5.2]].forEach(([r, y]) => {
    const p = new THREE.Mesh(new THREE.TorusGeometry(r, 0.022, 12, 48), trimMat);
    p.rotation.x = Math.PI / 2;
    p.position.set(0, y, 2.2);
    scene.add(p);
    const wire = new THREE.Mesh(
      new THREE.CylinderGeometry(0.006, 0.006, ROOM.h - y, 6),
      new THREE.MeshStandardMaterial({ color: 0xcbb591, roughness: 0.6 })
    );
    wire.position.set(r, y + (ROOM.h - y) / 2, 2.2);
    scene.add(wire);
  });

  // gallery portraits: entrance wall + the front of both side walls
  const artSpots = [
  ...
  let stopIndex = 0;
  const targetPos = new THREE.Vector3(...STOPS[0].pos);
  const lookCurrent = new THREE.Vector3(...STOPS[0].look);
  const targetLook = new THREE.Vector3(...STOPS[0].look);
  let yaw = 0, pitch = 0;
